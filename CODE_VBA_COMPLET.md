...
### Nouveau champ ajouté : `check_terrain`

```vba
' LIGNE 125-128 : Les deux champs check sont maintenant exportés
objLines.Add "    , check_terrain: " & BoolJS(ws.Cells(i, "R").Value)  ' NOUVEAU
objLines.Add "    , check: " & BoolJS(ws.Cells(i, "S").Value)          ' Existant
```

### Mapping des colonnes

| Champ JavaScript | Colonne Excel | Description |
|------------------|---------------|-------------|
...
const data_1 = [
  {
    ...
    , agent: "Jean Dupont"
    , tso: "TSO123"
    , debrief: true            // Débriefing - Colonne V
...
Par défaut : `G:\Web App Nico\En developpement\Suivi de contrôle\data_1.js`

Pour changer le chemin, modifiez la **ligne 28** du code :
```vba
...
| tso | U | TSO |
| debrief | V | Débriefing avec l'agent |
| commentaire | AE (31) | Commentaire |
| errors_count | 30 | Nombre d'erreurs |
| error_codes | 49-200 | Codes d'erreur cochés |
| tags | AE (31) | Tags extraits |
| history | 47 | Historique |
| has_comment | AE (31) | Présence de commentaire |

---

## 📝 NOTES

- La macro commence à lire à partir de la **ligne 10**
- Elle exporte toutes les lignes qui ont une date (colonne E)
- Les erreurs sont détectées dans les colonnes 49 à 200
- Les tags sont extraits automatiquement des commentaires entre crochets [TAG]
